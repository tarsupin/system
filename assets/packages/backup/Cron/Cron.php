...
/****** Get detailed list of Cron Tasks ******/
	...
	)				// RETURNS <array> the data for the cron task list, empty array on failure.
...
/****** Get data about a Cron Task ******/
	...
	(
		$cronID		// <int> The ID of the cron task to retrieve.
	)				// RETURNS <array> the data for the cron task, empty array on failure.
	
	// $cronData = Cron::getData($cronID);
	{
		if($results = Database::selectOne("SELECT * FROM cron WHERE id=? LIMIT 1", array($cronID)))
		{
			$results['args'] = Site::getArgs("CronTask", $results['id']);
		}
...
/****** Create a Cron Task ******/
	...
	(
		$title				// <str> The title of the task, useful only to humans.
		$method				// <str> The MyTasks::{method} or Task::{method} to call when activated.
		$args = array()		// <array> The array of arguments to pass to the cron method.
		$run_cycle = 600	// <int> Seconds until you repeat the task (default = 10 minutes).
		$date_start = 0		// <int> Timestamp of when to start the task. If 0, start now.
		$date_end = 0		// <int> Timestamp of when to end the task. If 0, don't end. If < now, run once.
	)						// RETURNS <bool> TRUE if task was created, FALSE on failure.
...
/****** Create a One-Time Cron Task ******/
	...
	(
		$title				// <str> The title of the task, useful only to humans.
		$method				// <str> The MyTasks::{method} or Task::{method} to call when activated.
		$args = array()		// <array> The array of arguments to pass to the task's function.
		$date_start = 0		// <int> Timestamp of when to run the task. If 0, run it right away.
...
/****** Run Cron Task ******/
	...
	(
		$id				// <int> The ID of the cron task you want to run.
		$reset = true	// <bool> TRUE to reset the task activation afterward, FALSE to not reset.
	)					// RETURNS <bool> TRUE on success, FALSE on failure.
...
/****** Process Cron Job (from Data) ******/
	...
	(
		$task			// <array> The cron task data.
		$reset = true	// <bool> TRUE if you want to reset the task activation, FALSE to not reset.
	)					// RETURNS <bool> TRUE on success, FALSE on failure.
...
used again, this function will remove it from the list.
	...
	(
		$id				// <int> The ID of the cron task that you want to reset.
		$date_nextRun	// <int> The next activation timer for the job.
		$run_cycle		// <int> The number of seconds to pass until you reset.
		$date_start		// <int> The timestamp to start at.
		$date_end		// <int> The timestamp to end at.
	)					// RETURNS <bool> TRUE on success, FALSE on failure.

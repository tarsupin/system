...
/****** Class Variables ******/
	...
	public $timeStart = 0;		// <int> Scheduled events cannot start prior to this timestamp
	public $timeEnd = 0;		// <int> If set to '0', the schedule has no date that it is required to stop at
	
	// Contains all of the rules for when the schedule activates
	public $triggers = array();		// <str:[int:mixed]> the schedule triggers (days, months, etc. to activate on)
	
	
/****** Class Constructor ******/
	public function __construct
	(
		$saveData = false	// <str> The saved data to rebuild a schedule (either JSON or custom data).
	)						// RETURNS <void>
...
/****** Save a schedule ******/
	...
	)				// RETURNS <str> JSON-minified string that can be used to restore a schedule.
...
/****** Specify the start time of the schedule ******/
	...
	(
		$start		// <int> The timestamp of when to start a schedule.
	)				// RETURNS <this>
...
/****** Compare a schedule to a timestamp ******/
	...
	(
		$timestamp = 0		// <int> The timestamp to compare the schedule with (default is current time).
	)						// RETURNS <str:mixed>
...
/****** Return a full list of months ******/
	...
	(
		$unitType	// <str> The type of unit list to retrieve.
	)				// RETURNS <int:int> list of the requested unit type.
...
/****** Help track down the next available time unit allowed ******/
	...
	(
		$timeUnitArray		// <int:int> Contains each time slot / values accepted for the time unit type.
	,	$trackerUnit		// <int> The value of the tracker time unit (the one you're matching against).
	,	&$update = false	// <bool> If this gets set to true, you progressed in the time sequence.
	,	$default = false	// <mixed> The default value to set if the time array is empty.
	)						// RETURNS <mixed>

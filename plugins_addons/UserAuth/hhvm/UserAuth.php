...
/****** Add a User under Auth Identification ******/
	...
	(
		int $authID			// <int> The AuthID to add the user to.
	,	int $uniID			// <int> The UniID to add to the Auth User.
	,	string $handle			// <str> The handle to associate with the UniID.
...
/****** Get List of Users ******/
	...
	(
		int $authID			// <int> The AuthID from which to retrieve a list of users.
	): array <int, array<str, str>>					// RETURNS <int:[str:str]> list of users owned by that AuthID, array() if none.
...
/****** Get Auth ID by User Handle ******/
	...
	(
		string $handle			// <str> The handle to use to retrieve the Auth ID.
	): int					// RETURNS <int> the Auth ID, or 0 on failure.
...
# expect the user to exist). If the user is detected, register them, and do so with UserAuth as well.
	...
	(
		mixed $user		// <mixed> UniID or User Handle that you want to silently register.
	): array <str, str>				// RETURNS <str:str> the user data that was registered, array() on failure.

...
/****** Get the User's Content Entries ******/
	...
	(
		int $uniID				// <int> The UniID whose content entries you are retrieving.
	,	int $startPos = 0		// <int> The starting position to retrieve rows from.
	,	int $rows = 20			// <int> The number of rows to retrieve.
	,	int $minStatus = 0		// <int> The minimum status allowed to retrieve.
	,	string $columns = "c.*"	// <str> The columns to retrieve during this query.
	): array <int, array<str, mixed>>						// RETURNS <int:[str:mixed]> The data associated with the article.

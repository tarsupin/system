...
/****** Public Variables ******/
	public static $transEncKey = "ujtk-uni-shop";		// <str> The encryption key for transactions.
...
# any input values, and instead just allow the user to click to pay that amount.
	...
	,	$haveFee = true		// <bool> TRUE if there is a standard fee for this transaction, FALSE if not.
	,	$imageURL = ""		// <str> The image to show for the transaction, if wanted.
	,	$minAmount = 0.00	// <float> The minimum amount that can be spent on the transaction.
	,	$maxAmount = 0.00	// <float> The maximum amount that can be spent on the transaction (0.00 is unlimited).
	,	$defAmount = 0.00	// <float> The default amount for this transaction (0.00 is no default).
	,	$opts = array()		// <int:float> An array of default values that you can select between.
	,	$custom = array()	// <str:mixed> A string array of custom data to return back.
	)						// RETURNS <str> A URL for a transaction.
...
/****** Charges a payment to a user instantly (one-click payment) ******/
	...
	(
		$uniID			// <int> The UniID of the user to charge credits.
	,	$amount			// <float> Sets how many credits was added to the recipient.
	,	$desc = ""		// <str> A brief description about the transaction's purpose.
	,	$fee = false	// <bool> TRUE to charge a fee for this, FALSE if not.
	)					// RETURNS <bool> TRUE on successful charge, FALSE on failure.
...
/****** Refunds a payment to a user instantly (one-click refund) ******/
	...
	,	$uniID				// <int> The Uni-Account to return the charge to.
	,	$desc = ""			// <str> A brief description about the transaction's purpose.
	)						// RETURNS <mixed>
...
# Note: This API will only function for trusted sites.
	...
	(
		$senderUniID		// <int> The Uni-Account of the user to charge.
	,	$receipientUniID	// <int> The Uni-Account of the user to receive.
	,	$amount				// <float> Sets how many credits was added to the recipient.
	,	$desc = ""			// <str> A brief description about the transaction's purpose.
...
# Note: This API will only function for trusted sites.
	...
	(
		$senderUniID		// <int> The Uni-Account of the user to charge.
	,	$receipientUniID	// <int> The Uni-Account of the user to receive.
	,	$tipType = "tip"	// <str> The type of tip (tiny, small, tip, big, large, huge, etc)
	)						// RETURNS <bool> TRUE on successful exchange, FALSE on failure.

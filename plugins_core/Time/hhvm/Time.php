...
/****** Convert a month string to a valid number ******/
	...
	(
		mixed $month	// <mixed> The month to convert.
	): int			// RETURNS <int> the numeric value of the month (e.g. 2 for "February").
...
/****** Convert a day of the week string to a valid number ******/
	...
	(
		mixed $day	// <mixed> The day of the week to convert.
	): int			// RETURNS <int> the numeric value of the day of the week (e.g. 3 for "Wednesday").
...
/****** Convert an hour string to a valid number ******/
	...
	(
		mixed $hour	// <mixed> The hour to convert.
	): int			// RETURNS <int> the numeric value of the hour (e.g. 22 for "10 pm").
...
/****** Return the current cycle ******/
	...
	(
		string $type = "Ym"	// <str> The type of cycle (date format) such as: "Ym" for Year+Month
	,	int $modifier = 0	// <int> The modifier, in seconds, to adjust the timestamp for.
	): int					// RETURNS <int> The resulting cycle after modification
...
/****** Return the current Swatch Time ******/
	...
	): int			// RETURNS <int> the current swatch time.
...
/****** Return the current UniQUE Time ******/
	...
	(
		int $match = -10	// <int> Set this to see if the unique time matches up.
	,	int $range = 2		// <int> The range (in roughly minutes) to allow for a match to be considered valid.
	): mixed					// RETURNS <mixed> the current swatch time.
...
/****** Return a list of timezones ******/
	...
	): array <str, str>			// RETURNS <str:str> a list of timezones.
...
/****** Fuzzy Time - Change a Timestamp to Simple Time Reference (i.e. "last week") ******/
	...
	(
		int $timestamp			// <int> The timestamp of the date to transfer to human-readable time.
	): string						// RETURNS <str> a simple, fuzzy time reference.
...
/****** Fuzzy Time (Future) - Private Helper ******/
	...
	(
		int $secondsUntil		// <int> The duration (in seconds) until the due date.
	,	int $timestamp			// <int> The time that the event is occuring.
	): string						// RETURNS <str> the fuzzy time (or a standard, formatted time).
...
/****** Fuzzy Time (Past) - Private Helper ******/
	...
	(
		int $secondsAgo			// <int> The duration (in seconds) after the due date.
	,	int $timestamp			// <int> The time that the event occurred.
	): string						// RETURNS <str> the fuzzy time (or a standard, formatted time).

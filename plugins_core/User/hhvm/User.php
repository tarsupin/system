...
/****** Get User Data ******/
	...
	(
		int $uniID			// <int> The Uni-Account to retrieve.
	,	string $columns = "*"	// <str> The columns you want to retrieve from the users database.
	): array <str, mixed>					// RETURNS <str:mixed> user data if retrieve was successful, FALSE if empty.
...
/****** Get UniID from a Handle ******/
	...
	(
		string $handle		// <str> The handle to look up.
	): int				// RETURNS <int> the UniID associated with the handle, or 0 if not found.
...
/****** Get User's Data from a Handle ******/
	...
	(
		string $handle				// <str> The handle to look up.
	,	string $columns = "uni_id"	// <str> The columns you want to retrieve from the users database.
	): array <str, mixed>						// RETURNS <str:mixed> user data associated with the handle provided, array() if empty.
...
/****** Login a User (using a valid Auth-API response) ******/
	...
	(
		array <str, mixed> $loginResponse		// <str:mixed> the API response that was provided from Auth.
	): int						// RETURNS <int> The UniID of the user created, 0 if failed.
...
/****** Register a User ******/
	...
	(
		int $uniID				// <int> The Uni-Account ID.
	,	string $handle				// <str> The handle for this Uni-Account.
	,	string $displayName = ""	// <str> The display name of the account.
	,	string $timezone = ""		// <str> The timezone of the account.
	): bool						// RETURNS <bool> TRUE if successful, FALSE if failed.
...
# expect the user to exist). If the user is detected, register them.
	...
	// User::silentRegister($user);
	{
		...
		if($response = Connect::to("unifaction", "UserData", $packet))
		{
...
/****** Return Clearance Values ******/
	...
	): array <int, str>					// RETURNS <int:str> array of clearance levels

...
/****** A generic "run" command to automate the entire Sync system ******/
	...
	(
		$pluginList = array()	// <int:str> The list of plugins to check for syncing. Default: all of them.
...
/****** Sync a plugin ******/
	...
	(
		$plugin			// <str> The plugin to synchronize.
	,	$force = false	// <bool> TRUE will force the sync to occur, FALSE will check if it's a valid time.
	)					// RETURNS <bool> TRUE if synchronized, FALSE if not.
...
/****** Check a plugin's tracker data ******/
	...
	(
		$plugin		// <str> The plugin to retrieve the last sync time for.
	)				// RETURNS <str:mixed> The tracker data for a plugin, array() on failure.
...
/****** Update a plugin's tracker data ******/
	...
	(
		$plugin				// <str> The plugin to set the sync time for.
	,	$trackerTime		// <int> The timestamp of the last time the plugin attempted a synchronization.
	,	$syncTime			// <int> The final sync time received (where to start the next sync).
	,	$delay = -1			// <int> The amount of time that must pass before syncing again (default is 5 minutes).
	,	$overwrite = false	// <bool> TRUE if you force the maximum time, FALSE if overwriting is possible (can set lower).
	)						// RETURNS <bool> TRUE on success, FALSE on failure.

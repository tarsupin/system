...
/****** Plugin Variables ******/
	...
	public static array <str, int> $illegalChars = array();		// <str:int>
...
# will attempt to create a warning for the admins of a potential hack attempt.
	...
	(
		string $valueToSanitize	// <str> The value you're going to sanitize.
	,	string $charsAllowed		// <str> A list of specific characters to add to the whitelist.
	,	bool $strict = false		// <bool> TRUE to end sanitization once a non-matching character is found.
	): string						// RETURNS <str> the sanitized value (as an acceptably formatted word).
...
# Sanitizes input so that it returns everything up until the first non-matching character is found.
	...
	(
		string $valueToSanitize		// <str> The value you're going to sanitize.
	,	string $useMethod				// <str> The method that you're going to use.
	,	string $extraChars = ""		// <str> A list of specific characters to add to the whitelist.
	): string							// RETURNS <str> input until the first non-matching character.
...
/****** Sanitize Methods ******/
	...
	,	array <int, mixed> $args		// <int:mixed> Additional arguments being passed.
	): string				// RETURNS <str> input until the first non-matching character.
...
/****** Sanitize Number ******/
	public static function number	<T> // <T>
	(
		mixed $numberToSanitize		// <mixed> The number you're going to sanitize.
	,	T $minRange = 0			// <T> The minimum range allowed.
	,	T $maxRange = 9999999999	// <T> The maximum range allowed.
	,	bool $isFloat = false		// <bool> TRUE if the value is a float rather than an integer.
	): mixed							// RETURNS <mixed> the sanitized value that results after sanitizing.
...
# Severe warnings may occur if the user attempts to enter parent paths ("../") or uses null bytes.
	...
	(
		string $valueToSanitize	// <str> The input to sanitize
	,	mixed $extAllowed = ""	// <mixed> The extensions that are allowed. Default allows any extensions.
	): string						// RETURNS <str> sanitized file path, or "" on failure.
...
#	10	= Definitely an attack - only a trained penetration tester would do this
	...
	(
		string $unsafeContent		// <str> The illegal input that was captured
	,	string $threatText = ""	// <str> The type of warning to announce
	,	int $severity = 0		// <int> The severity level of this particular input
	,	int $traceDepth = 0		// <int> Internal value to aid in identifying how many additional backtrace steps are needed
	): void						// RETURNS <void>

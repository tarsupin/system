...
/****** Transfer data from one table to another ******/
	...
	(
		string $sourceTable		// <str> The original table that you're backing up.
	,	string $destinationTable	// <str> The table name that you're backing up to.
	,	string $sqlWhere = ""		// <str> The WHERE clause entry for the SQL.
	,	array <int, mixed> $sqlArray = array()	// <int:mixed> The SQL array to populate the WHERE clause.
	,	int $limit = 1000		// <int> The number of rows to limit (to avoid overwhelm). 0 is unlimited.
	,	bool $move = false		// <bool> Set to TRUE if you want to move the data rather than copy it.
